...
export default function Hero() {
  ...
  // Pre-calculate positions for floating particles to avoid Math.random() during render
  ...
  return (
    <section
      ref={heroRef}
      className="relative py-2 flex flex-col justify-center bg-gradient-to-br from-slate-900 via-indigo-900 to-purple-900 dark:from-slate-950 dark:via-indigo-950 dark:to-purple-950 text-white overflow-hidden"
    ...
    >
      ...
      {floatingElements.map((element, index) => (
        <div
          key={index}
          className={`absolute w-8 h-8 md:w-12 md:h-12 ${element.color} rounded-xl md:rounded-2xl flex items-center justify-center text-white shadow-xl backdrop-blur-sm border border-white/15 dark:border-white/8 hidden md:flex opacity-80`}
          style={{
            ...element.position,
      ...
      <div className="relative z-10 flex-1 flex flex-col justify-center px-4 py-0 md:py-safe">
        <div className="w-full max-w-xs mx-auto sm:max-w-sm md:max-w-4xl lg:max-w-6xl">
          ...
          <div className="flex flex-col md:flex-row justify-center items-center gap-6 md:gap-12 mb-7 md:mb-9">
            ...
            <div className="hidden md:flex justify-center">
              <div className="relative group cursor-pointer">
                {/* Outer glow effect */}
                <div className="absolute inset-0 w-32 h-32 md:w-40 md:h-40 bg-gradient-to-r from-emerald-400/20 via-blue-400/20 to-purple-400/20 rounded-full blur-xl group-hover:blur-2xl transition-all duration-700"></div>

                {/* Enhanced rotating rings with different speeds and colors */}
                <div className="absolute inset-0 w-32 h-32 md:w-40 md:h-40">
                  <div className="absolute inset-2 border-2 border-gradient-to-r from-emerald-400/30 to-blue-400/30 rounded-full"></div>
                  <div className="absolute inset-3 border-2 border-gradient-to-r from-blue-400/25 to-purple-400/25 rounded-full"></div>
                  <div className="absolute inset-4 border border-gradient-to-r from-purple-400/20 to-pink-400/20 rounded-full"></div>
                  <div className="absolute inset-5 border border-white/15 dark:border-white/8 rounded-full"></div>
                  <div className="absolute inset-6 border border-white/10 dark:border-white/5 rounded-full"></div>
                </div>
                ...
                <div className="absolute inset-0 w-32 h-32 md:w-40 md:h-40">
                  {particlePositions.map((pos, i) => (
                    <div
                      key={i}
                      className="absolute w-1 h-1 bg-gradient-to-r from-emerald-400 to-blue-400 rounded-full opacity-60"
                      style={{
                        top: pos.top,
                        left: pos.left,
                ...
                <div className="relative w-32 h-32 md:w-40 md:h-40 bg-gradient-to-br from-white/15 via-emerald-400/5 to-blue-400/5 dark:from-white/8 dark:via-emerald-400/3 dark:to-blue-400/3 backdrop-blur-xl rounded-full flex flex-col items-center justify-center border-2 border-white/20 dark:border-white/10 hover:scale-110 hover:border-emerald-400/40 transition-all duration-700 shadow-2xl group-hover:shadow-emerald-400/20">
                  ...
                  <div className="relative z-10 mb-2 transform group-hover:scale-125 transition-all duration-500">
                    <TrendingUp className="w-8 h-8 md:w-10 md:h-10 text-emerald-400 dark:text-emerald-500 drop-shadow-lg" />
                  ...
                  {/* Pulsing inner ring */}
                  <div className="absolute inset-6 border border-emerald-400/20 rounded-full group-hover:border-emerald-400/40 transition-all duration-500"></div>

                  {/* Success indicators */}
                  <div className="absolute -top-1 -right-1 w-3 h-3 md:w-4 md:h-4 bg-gradient-to-r from-emerald-400 to-green-500 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-500">
          ...
          <div className="flex flex-col gap-3 justify-center items-center mb-7 md:mb-9">
